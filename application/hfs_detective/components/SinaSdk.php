...
class SinaSdk
{
	...
	public function getOauthLink($backurl, $state='front', $akey = WB_AKEY, $skey = WB_SKEY) {
		if(empty($backurl)) {
			return false;
		}
		return $this->oauth->getAuthorizeURL($backurl, 'code', $state);
        }
	...
	public function short_url($url) {
		return $this->oauth->get( 'short_url/shorten', array('url_long'=>$url));
	}
	 ...
	 * 搜索某一话题下的微博
	 * 
	 * 对应API：{@link http://open.weibo.com/wiki/2/search/topics search/topics}
	 * 
	 * @param  String 	$q		搜索的话题
	 * @param  Int 		$count	但页返回记录条数,默认50
	 * @param  Int 		$page	返回结果的页码，默认为1。
	 * @return Array
	 */
	function search_topics($q, $count=50, $page=1)
	{
		$params = array();
		$params['q'] 		= $q;
		$params['count'] 	= $count;
	 ...
	 * 获取指定坐标的位置信息
	 * 
	 * 对应API：{@link http://open.weibo.com/wiki/2/2/location/geo/geo_to_address location/geo/geo_to_address}
	 * 
	 * @param  float $x 纬度
	 * @param  float $y 经度
	 * @return Array <mixed, string>
	 */
	function geo_to_address($x, $y)
	{
		$params = array(); 
		$params['coordinate'] = "{$x}, {$y}";
		return $this->oauth->get('location/geo/geo_to_address', $params );
	}
	 ...
	 * 获取用户图片列表列表
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/place/users/photos}
	 *
	 ...
	 * @return array
	 */
	function get_user_pics( $uid, $page = 1 , $count = 50) {
		$params = array();

		$params['uid'] = $uid;
		$params['count'] = $count;
		$params['page'] = $page;
		return $this->oauth->get( 'place/users/photos', $params );
	}
	
	/**
	 * 给一个或多个用户发送一条新的状态通知
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/notification/send}
	 *
	 * @param int $uids 接收通知的用户UID，支持1-100个用户，用逗号分隔。(必填)
	 * @param int $tpl_id  发送通知所对应的模版ID，必须是通过审核，且启用中的模版。(必填)
	 * @param string $objects1 通知的自定义变量1，可以是人也可以是系统实物，支持@用户昵称，最多不超过32个字节。
	 * @param int $objects1_count 通知的自定义变量1的数量，正整数，最多不超过2个字节，支持0-99。
	 * @param string $objects2 通知的自定义变量2，可以是人也可以是系统实物，支持@用户昵称，最多不超过32个字节。
	 * @param int $objects2_count 通知的自定义变量2的数量，正整数，最多不超过2个字节，支持0-99。
	 * @param string $objects3 通知的自定义变量3，可以是人也可以是系统实物，支持@用户昵称，最多不超过32个字节。
	 * @param int $objects3_count 通知的自定义变量3的数量，正整数，最多不超过2个字节，支持0-99。
	 * @param int $action_url 通知的处理链接，支持短链接，最多不超过20个字节。
	 * @return array
	 */
	function send_notice( $uids, $tpl_id, $objects1 = '', $objects1_count = 0, $objects2 = '', $objects2_count = 0, $objects3 = '', $objects3_count = 0, $action_url = '') {
		$params = array();

		$params['uids']           = $uids;
		$params['tpl_id']         = $tpl_id;
		//$params['objects1']       = $objects1;
		//$params['objects1_count'] = $objects1_count;
		//$params['objects2']       = $objects2;
		//$params['objects2_count'] = $objects2_count;
		//$params['objects3']       = $objects3;
		//$params['objects3_count'] = $objects3_count;
